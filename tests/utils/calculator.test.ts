...
Deno.test("sumItems", async (t) => {
  await t.step("calulates sum of income and expense", () => {
    const items = [
      { title: "item1", amount: 100 },
      { title: "item2", amount: 100 },
      { title: "item3", amount: -100 },
      { title: "item4", amount: -100 },
    ];
...
Deno.test("calculateGrade", async (t) => {
  await t.step("returns A when ratio is less than 10", () => {
    const items = [
      { title: "item1", amount: 1000 },
      { title: "item2", amount: -99 },
    ];
    const grade = calculateGrade(items);
    assertEquals(grade, "A");
  });
  await t.step("returns B when ratio is greater than 10 and less than 30", () => {
    const items = [
      { title: "item1", amount: 1000 },
      { title: "item2", amount: -100 },
    ];
    ...
    items.push({ title: "item2", amount: -200 })
    grade = calculateGrade(items);
    assertEquals(grade, "B");
  });
  await t.step("returns C when ratio is greater than 30 and less than 50", () => {
    const items = [
      { title: "item1", amount: 1000 },
      { title: "item2", amount: -500 },
    ];
    const grade = calculateGrade(items);
